var gB=["a","abbr","acronym","address","applet","area","article","aside","audio","b","base","basefont","bdi","bdo","big","blockquote","body","br","button","canvas","caption","center","cite","code","col","colgroup","data","datalist","dd","del","details","dfn","dialog","dir","div","dl","dt","em","embed","fieldset","figcaption","figure","font","footer","form","frame","frameset","h1","h2","h3","h4","h5","h6","head","header","hgroup","hr","html","i","iframe","img","input","ins","kbd","label","legend","li","link","main","map","mark","menu","meta","meter","nav","noframes","noscript","object","ol","optgroup","option","output","p","param","picture","pre","progress","q","rp","rt","ruby","s","samp","script","search","section","select","slot","small","source","span","strike","strong","style","sub","summary","sup","table","tbody","td","template","textarea","tfoot","th","thead","time","title","tr","track","tt","u","ul","var","video","wbr"],JB=["animate","animateMotion","animateTransform","circle","clipPath","defs","desc","ellipse","feBlend","feColorMatrix","feComponentTransfer","feComposite","feConvolveMatrix","feDiffuseLighting","feDisplacementMap","feDistantLight","feDropShadow","feFlood","feFuncA","feFuncB","feFuncG","feFuncR","feGaussianBlur","feImage","feMerge","feMergeNode","feMorphology","feOffset","fePointLight","feSpecularLighting","feSpotLight","feTile","feTurbulence","filter","foreignObject","g","image","line","linearGradient","marker","mask","metadata","mpath","path","pattern","polygon","polyline","radialGradient","rect","set","stop","svg","symbol","text","textPath","tspan","use","view"],NB={SvgA:"a",SvgScript:"script",SvgStyle:"style",SvgSwitch:"switch",SvgTitle:"title"},FB=["annotation","annotation-xml","maction","math","merror","mfrac","mi","mmultiscripts","mn","mo","mover","mpadded","mphantom","mprescripts","mroot","mrow","ms","mspace","msqrt","mstyle","msub","msubsup","msup","mtable","mtd","mtext","mtr","munder","munderover","semantics"],h,UB=(B)=>{if(!h)h=document.createElement("textarea");return h.innerHTML=B,h.value},CB=(B)=>{try{return decodeURIComponent(B)}catch{return B}},mB=(B)=>{return B.replace(/\\u[\dA-Fa-f]{4}/g,(M)=>String.fromCharCode(parseInt(M.slice(2),16))).replace(/\\x[\dA-Fa-f]{2}/g,(M)=>String.fromCharCode(parseInt(M.slice(2),16)))},hB=()=>{let B=0;return{getNewId:()=>++B,resetIdCounter:()=>B=0}},jB=hB(),S={currentIs:(B)=>window._currentAppPhase===B,start:(B)=>{window._currentAppPhase=B,console.log(`Current phase is ${B}`)}},qB=(B,M,Q)=>{let W=B;W=UB(W),W=CB(W),W=mB(W);let X=W.trim().toLowerCase();for(let Z of M)if(Z.test(X))throw Q;return B},dB=(B)=>qB(B,[/^javascript\s*:/i,/^data\s*:/i,/^vbscript\s*:/i,/^file\s*:/i],'The href attribute value starting with one of "javascript:", "data:", "vbscript:" or "file:" is not allowed.'),cB=(B)=>qB(B,[/url\s*\(/i,/expression\s*\(/i,/javascript\s*:/i,/data\s*:/i,/vbscript\s*:/i,/file\s*:/i],'The style attribute value starting with one of "url(..", "expression(..", "javascript:", "data:", "vbscript:" or "file:" is not allowed.'),sB=(B,M)=>{if(B==="href"){if(typeof M==="boolean")throw"The value of 'href' attribute should not be a boolean";return dB(M||"")}if(B==="style"){if(typeof M==="boolean")throw"The value of 'style' attribute should not be a boolean";return cB(M||"")}return M},c=(()=>{let B=null,M=new Map,Q={addReceiver(W){B=W;try{W.run()}catch(X){throw Q.removeReceiver(W),X}finally{B=null}},removeReceiver(W){M.forEach((X,Z)=>{if(X.delete(W),X.size===0)M.delete(Z)})},ignoreSignalsRegistration(W){let X=B;B=null;try{return W()}finally{B=X}},notifySignalRegistration(W){if(B)if(M.has(W))M.get(W).add(B);else{let X=new Set([B]);M.set(W,X)}},notifySignalUpdate(W){M.get(W)?.forEach((Z)=>{Z.run()})}};return Q})(),a=(B)=>{let M=Object.entries(B).sort((Q,W)=>Q[0].localeCompare(W[0]));return M.forEach(([Q,W],X)=>{if(W&&typeof W==="object"&&!Array.isArray(W))M[X]=[Q,a(W)]}),Object.fromEntries(M)},o=(B)=>{if(typeof B!=="object"||B===null||Array.isArray(B))return!1;return Object.prototype.toString.call(B)==="[object Object]"},T=(B)=>{if(Array.isArray(B)){let Q=[...B],W=[];return Q.forEach((X)=>{W.push(T(X))}),W}if(o(B)){let Q={...B},W={};return Object.keys(Q).forEach((X)=>{W[X]=T(Q[X])}),W}return B},YB=(B,M="index")=>B.map((Q,W)=>({[M]:W,value:Q})),oB=(B,M)=>{let Q=a(B),W=a(M),X=Object.keys(Q),Z=Object.keys(W);if(X.length!==Z.length)return!1;for(let _ of X)if(!Z.includes(_)||!MB(Q[_],W[_]))return!1;return!0},pB=(B,M)=>{if(B.length!==M.length)return!1;if(B.length===0)return!0;for(let Q=0;Q<B.length;Q++)if(!MB(B[Q],M[Q]))return!1;return!0},MB=(B,M)=>{if(typeof B!==typeof M)return!1;if(Array.isArray(B))return pB(B,M);if(B===null||M===null)return B===M;if(typeof B==="object"&&!(B instanceof Set))return oB(B,M);if(typeof B==="bigint"||typeof B==="number"||typeof B==="string"||typeof B==="boolean")return B===M;return B===M},iB=(B,M,Q)=>{let X=YB(T(B),"index");return YB(T(M),"index").map((_)=>{let H="add",F=-1,K=_.value;return X.some((U,v)=>{if(H=MB(U.value,_.value)?U.index===_.index?"idle":"shuffle":Q&&U.value[Q]!==void 0&&U.value[Q]===_.value[Q]?"update":"add",H!=="add")return F=U.index,X.splice(v,1),!0;return!1}),{type:H,oldIndex:F,value:K}})},V=(B)=>["source-signal","derived-signal"].includes(B?.type);function w(B){return V(B)?B.value:B}var G=(...B)=>B.map((M)=>w(M)),rB=(B)=>{let M=(Q)=>B.mutateWith((W)=>{let X=Array.from(W);return Q(X),X});return{concat:(...Q)=>B.mutateWith((W)=>W.concat(...G(...Q))),copyWithin:(...Q)=>M((W)=>W.copyWithin(...G(...Q))),fill:(...Q)=>M((W)=>W.fill(...G(...Q))),filter:(...Q)=>B.mutateWith((W)=>{return W.filter(...G(...Q))}),pop:(...Q)=>M((W)=>W.pop(...G(...Q))),push:(...Q)=>M((W)=>W.push(...G(...Q))),shift:(...Q)=>M((W)=>W.shift(...G(...Q))),toReversed:(...Q)=>M((W)=>W.reverse(...G(...Q))),toSorted:(...Q)=>M((W)=>W.sort(...G(...Q))),toSpliced:(...Q)=>M((W)=>W.splice(...G(...Q))),unshift:(...Q)=>M((W)=>W.unshift(...G(...Q)))}},nB=(B)=>{return{at:(...M)=>$(()=>B.value.at(...G(...M))),concat:(...M)=>$(()=>B.value.concat(...G(...M))),every:(...M)=>$(()=>B.value.every(...G(...M))),entries:()=>$(()=>B.value.entries()),filter:(...M)=>$(()=>B.value.filter(...G(...M))),find:(...M)=>$(()=>B.value.find(...G(...M))),findIndex:(...M)=>$(()=>B.value.findIndex(...G(...M))),findLast:(...M)=>$(()=>B.value.findLast(...G(...M))),findLastIndex:(...M)=>$(()=>B.value.findLastIndex(...G(...M))),flat:(M)=>$(()=>B.value.flat(w(M))),flatMap:(M,Q)=>$(()=>B.value.flatMap(M,w(Q))),forEach:(...M)=>$(()=>{B.value.forEach(...G(...M))}),includes:(...M)=>$(()=>B.value.includes(...G(...M))),indexOf:(...M)=>$(()=>B.value.indexOf(...G(...M))),join:(...M)=>$(()=>B.value.join(...G(...M))),keys:()=>$(()=>B.value.keys()),lastIndexOf:(...M)=>$(()=>B.value.lastIndexOf(...G(...M))),length:()=>$(()=>B.value.length),map:(M,Q)=>$(()=>B.value.map(M,w(Q))),reduce:(M,...Q)=>$(()=>Q.length===0?B.value.reduce(M):B.value.reduce(M,w(Q[0]))),reduceRight:(M,...Q)=>$(()=>Q.length===0?B.value.reduceRight(M):B.value.reduceRight(M,w(Q[0]))),some:(...M)=>$(()=>B.value.some(...G(...M))),slice:(...M)=>$(()=>B.value.slice(...G(...M))),toLocaleString:(...M)=>$(()=>B.value.toLocaleString(...G(...M))),toReversed:(...M)=>$(()=>B.value.toReversed(...G(...M))),toSorted:(...M)=>$(()=>B.value.toSorted(...G(...M))),toSpliced:(...M)=>$(()=>B.value.toSpliced(...G(...M))),values:()=>$(()=>B.value.values()),with:(...M)=>$(()=>B.value.with(...G(...M)))}},tB=(B)=>{return{lastItem:()=>{return $(()=>{return T(B.value).pop()})},partition:(...M)=>{let Q=$(()=>{let[X,Z]=G(...M);return B.value.filter(X,Z)}),W=$(()=>{let[X,Z]=G(...M);return B.value.filter((_,H,F)=>!X.call(Z,_,H,F))});return[Q,W]}}},DB=(B)=>({...nB(B),...tB(B)}),lB=(B)=>({mutate:{...rB(B)},...DB(B)}),aB=(B)=>({toggle:()=>B.mutateWith((M)=>!M)}),eB=(B)=>({mutate:{...aB(B)}}),BM=(B)=>{return{toExponential:(...M)=>$(()=>B.value.toExponential(...G(...M))),toFixed:(...M)=>$(()=>B.value.toFixed(...G(...M))),toPrecision:(...M)=>$(()=>B.value.toPrecision(...G(...M))),toLocaleString:(M,Q)=>$(()=>B.value.toLocaleString(w(M),w(Q)))}},MM=(B)=>{return{toConfined:(M,Q)=>$(()=>{let W=w(M),X=w(Q);return B.value<W?W:B.value>X?X:B.value})}},LB=(B)=>({...BM(B),...MM(B)}),QM=(B)=>({set:(M)=>B.mutateWith((Q)=>({...Q,...M}))}),KB=(B)=>{return{keys:()=>$(()=>Object.keys(B.value)),get:(M)=>$(()=>B.value[M]),props:()=>{let M={};return Object.keys(B.value).forEach((Q)=>{M[Q]=$(()=>B.value[Q])}),M}}},WM=(B)=>({mutate:{...QM(B)},...KB(B)}),RB=(B)=>B.trim().replace(/\s+/g," "),XM=(B)=>{return{concat:function(...M){B.mutateWith((Q)=>Q.concat(...G(...M)))},deepTrim:function(){B.mutateWith((M)=>RB(M))},padEnd:function(...M){B.mutateWith((Q)=>Q.padEnd(...G(...M)))},padStart:function(...M){B.mutateWith((Q)=>Q.padStart(...G(...M)))},repeat:function(...M){B.mutateWith((Q)=>Q.repeat(...G(...M)))},replace:function(...M){B.mutateWith((Q)=>{let[W,X]=G(...M);return Q.replace(W,X)})},replaceAll:function(...M){B.mutateWith((Q)=>{let[W,X]=G(...M);return Q.replaceAll(W,X)})},slice:function(...M){B.mutateWith((Q)=>Q.slice(...G(...M)))},substring:function(...M){B.mutateWith((Q)=>Q.substring(...G(...M)))},trim:function(...M){B.mutateWith((Q)=>Q.trim(...G(...M)))},trimEnd:function(...M){B.mutateWith((Q)=>Q.trimEnd(...G(...M)))},trimStart:function(...M){B.mutateWith((Q)=>Q.trimStart(...G(...M)))},toLocaleLowerCase:function(...M){B.mutateWith((Q)=>Q.toLocaleLowerCase(...G(...M)))},toLocaleUpperCase:function(...M){B.mutateWith((Q)=>Q.toLocaleUpperCase(...G(...M)))},toLowerCase:function(...M){B.mutateWith((Q)=>Q.toLowerCase(...G(...M)))},toUpperCase:function(...M){B.mutateWith((Q)=>Q.toUpperCase(...G(...M)))}}},YM=(B)=>{return{at:(...M)=>$(()=>B.value.at(...G(...M))),charAt:(...M)=>$(()=>B.value.charAt(...G(...M))),charCodeAt:(...M)=>$(()=>B.value.charCodeAt(...G(...M))),codePointAt:(...M)=>$(()=>B.value.codePointAt(...G(...M))),concat:(...M)=>$(()=>B.value.concat(...G(...M))),endsWith:(...M)=>$(()=>B.value.endsWith(...G(...M))),includes:(...M)=>$(()=>B.value.includes(...G(...M))),indexOf:(...M)=>$(()=>B.value.indexOf(...G(...M))),lastIndexOf:(...M)=>$(()=>B.value.lastIndexOf(...G(...M))),padEnd:(...M)=>$(()=>B.value.padEnd(...G(...M))),padStart:(...M)=>$(()=>B.value.padStart(...G(...M))),repeat:(...M)=>$(()=>B.value.repeat(...G(...M))),slice:(...M)=>$(()=>B.value.slice(...G(...M))),startsWith:(...M)=>$(()=>B.value.startsWith(...G(...M))),substring:(...M)=>$(()=>B.value.substring(...G(...M))),trim:(...M)=>$(()=>B.value.trim(...G(...M))),trimEnd:(...M)=>$(()=>B.value.trimEnd(...G(...M))),trimStart:(...M)=>$(()=>B.value.trimStart(...G(...M))),length:()=>$(()=>B.value.length),localeCompare:(...M)=>$(()=>B.value.localeCompare(...G(...M))),normalize:(...M)=>$(()=>B.value.normalize(w(...G(...M)))),replace:(...M)=>$(()=>{let[Q,W]=G(...M);return B.value.replace(Q,W)}),replaceAll:(...M)=>$(()=>{let[Q,W]=G(...M);return B.value.replaceAll(Q,W)}),search:(...M)=>$(()=>B.value.search(...G(...M))),split:(...M)=>$(()=>{let[Q,W]=G(...M);return B.value.split(Q,W)}),toLocaleLowerCase:(...M)=>$(()=>B.value.toLocaleLowerCase(...G(...M))),toLocaleUpperCase:(...M)=>$(()=>B.value.toLocaleUpperCase(...G(...M))),toLowerCase:(...M)=>{return $(()=>B.value.toLowerCase(...G(...M)))},toUpperCase:(...M)=>{return $(()=>B.value.toUpperCase(...G(...M)))}}},ZM=(B)=>{return{deepTrim:()=>{return $(()=>RB(B.value))}}},zB=(B)=>({...YM(B),...ZM(B)}),_M=(B)=>({mutate:{...XM(B)},...zB(B)}),$M=(B,M)=>{let Q=M===void 0?B.nonReactiveValue:M;if(Array.isArray(Q))return DB(B);if(o(Q))return KB(B);if(typeof Q==="string")return zB(B);if(typeof Q==="number")return LB(B);return{}},wM=(B,M)=>{let Q=M===void 0?B.nonReactiveValue:M;if(Array.isArray(Q))return lB(B);if(o(Q))return WM(B);if(typeof Q==="string")return _M(B);if(typeof Q==="number")return LB(B);if(typeof Q==="boolean")return eB(B);return{}},GM=(B)=>{return{then:(M,Q)=>{return $(()=>{let W=w(M),X=w(Q);return B()?W:X})}}},e=(B,M)=>{let Q=(W)=>M?GM(W):$(W);return{truthy:()=>Q(()=>!!B()),falsy:()=>Q(()=>!B()),equalTo:(W)=>Q(()=>B()===w(W)),notEqualTo:(W)=>Q(()=>B()!==w(W)),greaterThan:(W)=>Q(()=>B()>w(W)),greaterThanOrEqualTo:(W)=>Q(()=>B()>=w(W)),smallerThan:(W)=>Q(()=>B()<w(W)),smallerThanOrEqualTo:(W)=>Q(()=>B()<=w(W))}},ZB=(B,M)=>{return{length:e(B,M)}},AB=(B)=>{let M=()=>w(B),Q=()=>{let W=w(B);if(typeof W==="string"||Array.isArray(W))return W.length;return NaN};return{is:{...e(M,!1),...ZB(Q,!1)},if:{...e(M,!0),...ZB(Q,!0)},or:(W)=>{return $(()=>{let X=w(W);return w(B)||X})},toString:()=>{return $(()=>{let W=w(B);if(W===null)return"null";if(W===void 0)return"undefined";if(o(W))return JSON.stringify(W);return W.toString()})}}},vB=(()=>{let B=0;return{get newID(){return++B}}})(),C=(B)=>{let M=vB.newID,Q={get id(){return M},run(){B()},dispose(){c.removeReceiver(Q)}};return c.addReceiver(Q),Q},x=(B,M)=>{let Q=vB.newID,W=void 0,X=T(B),Z={get type(){return"source-signal"},get id(){return Q},get prevValue(){return W},get nonReactiveValue(){return X},get value(){return c.notifySignalRegistration(Z),T(X)},set value(_){if(_===X){console.warn(`Unncessary assignment to sourceSignal with ID - ${Q}`);return}W=X,X=_,c.notifySignalUpdate(Z)},mutateWith(_){let H=_(X);this.value=H}};return Object.assign(Z,AB(Z)),Object.assign(Z,wM(Z,M)),Z},$=(B,M)=>{let Q=x(void 0),W=C(()=>{Q.value=B(Q.nonReactiveValue)}),X={get type(){return"derived-signal"},get prevValue(){return Q.prevValue},get nonReactiveValue(){return Q.nonReactiveValue},get value(){return Q.value},dispose(){W.dispose()}};return Object.assign(X,AB(X)),Object.assign(X,$M(X,M)),X},D=(B)=>{let M=typeof B==="function"?B:()=>w(B);return{...{get result(){return $(()=>M())},get truthy(){return $(()=>!!M())},get falsy(){return $(()=>!M())},get truthyFalsyPair(){return $(()=>{let W=!!M();return[W,!W]})},then:(W,X)=>$(()=>{return M()?w(W):w(X)})},or:(W)=>D(()=>{return M()||w(W)}),orNot:(W)=>D(()=>{return M()||!w(W)}),and:(W)=>D(()=>{return M()&&w(W)}),andNot:(W)=>D(()=>{return M()&&!w(W)}),equals:(W)=>D(()=>{return M()===w(W)}),notEquals:(W)=>D(()=>{return M()!==w(W)}),orBothEqual:(W,X)=>D(()=>{let Z=M(),_=w(W)===w(X);return Z||_}),orBothUnequal:(W,X)=>D(()=>{let Z=M(),_=w(W)!==w(X);return Z||_}),andBothEqual:(W,X)=>D(()=>{let Z=M(),_=w(W)===w(X);return Z&&_}),andBothUnequal:(W,X)=>D(()=>{let Z=M(),_=w(W)!==w(X);return Z&&_}),orThisIsLT:(W,X)=>D(()=>{let Z=M(),_=w(W)<w(X);return Z||_}),orThisIsLTE:(W,X)=>D(()=>{let Z=M(),_=w(W)<=w(X);return Z||_}),orThisIsGT:(W,X)=>D(()=>{let Z=M(),_=w(W)>w(X);return Z||_}),orThisIsGTE:(W,X)=>D(()=>{let Z=M(),_=w(W)>=w(X);return Z||_}),andThisIsLT:(W,X)=>D(()=>{let Z=M(),_=w(W)<w(X);return Z&&_}),andThisIsLTE:(W,X)=>D(()=>{let Z=M(),_=w(W)<=w(X);return Z&&_}),andThisIsGT:(W,X)=>D(()=>{let Z=M(),_=w(W)>w(X);return Z&&_}),andThisIsGTE:(W,X)=>D(()=>{let Z=M(),_=w(W)>=w(X);return Z&&_})}},I=(B)=>{let M=typeof B==="function"?B:()=>w(B);return{...D(B),get result(){return $(M)},add:(Q)=>I(()=>{return M()+w(Q)}),sub:(Q)=>I(()=>{return M()-w(Q)}),mul:(Q)=>I(()=>{return M()*w(Q)}),div:(Q)=>I(()=>{return M()/w(Q)}),mod:(Q)=>I(()=>{return M()%w(Q)}),pow:(Q)=>I(()=>{return M()**w(Q)}),isBetween:(Q,W,X=!0,Z=!0)=>D(()=>{let _=M(),H=w(Q),F=w(W),K=X?_>=H:_>H,U=Z?_<=F:_<F;return K&&U}),isLT:(Q)=>D(()=>M()<w(Q)),isLTE:(Q)=>D(()=>M()<=w(Q)),isGT:(Q)=>D(()=>M()>w(Q)),isGTE:(Q)=>D(()=>M()>=w(Q))}},HM=(B)=>{let M=typeof B==="function"?B:()=>w(B);return{...D(B),lengthBetween:(Q,W,X=!0,Z=!0)=>D(()=>{let H=M().length,F=w(Q),K=w(W),U=X?H>=F:H>F,v=Z?H<=K:H<K;return U&&v}),lengthEquals:(Q)=>D(()=>{return M().length===w(Q)}),lengthNotEquals:(Q)=>D(()=>{return M().length!==w(Q)}),lengthLT:(Q)=>D(()=>{return M().length<w(Q)}),lengthLTE:(Q)=>D(()=>{return M().length<=w(Q)}),lengthGT:(Q)=>D(()=>{return M().length>w(Q)}),lengthGTE:(Q)=>D(()=>{return M().length>=w(Q)})}},r=(B)=>{let Q=(typeof B==="function"?B:()=>w(B))();return typeof Q==="number"?I(B):typeof Q==="string"||Array.isArray(Q)?HM(B):D(B)},A=(B,...M)=>$(()=>{return B.reduce((Q,W,X)=>{let Z,_=M[X];if(typeof _==="function")Z=_()??"";else if(V(_))Z=_.value??"";else Z=_??"";return`${Q}${W}${Z.toString()}`},"")}),QB=(B)=>Array.isArray(B),s=(B)=>!isNaN(B?.nodeID)&&B?.nodeID>0,JM=(B)=>typeof B==="function"&&B.isMayaNodeGetter===!0,g=(B)=>B===void 0||typeof B==="string"||JM(B),FM=(B)=>QB(B)&&B.every((M)=>g(M)),UM=(B)=>!V(B)&&(g(B)||FM(B)),BB=(B)=>V(B)&&g(B.value),EB=(B)=>QB(B)&&B.every((M)=>g(M)||BB(M)),jM=(B)=>!V(B)&&(g(B)||EB(B)),PB=(B)=>V(B)&&UM(B.value),VB=(B)=>jM(B)||PB(B),_B=!1,qM={},N={},DM=globalThis.MutationObserver,LM=new DM((B)=>{B.forEach((M)=>{if(M.type==="childList")M.addedNodes.forEach((Q)=>{if(s(Q)){let W=Q,X=W.nodeID;if(N[X])delete N[X];else qM[X]=W.tagName}}),M.removedNodes.forEach((Q)=>{if(s(Q)){let W=Q,X=W.nodeID,Z=W.unmountListener;if(Z)N[X]={mayaNode:W,unmountListener:Z}}})}),Object.entries(N).forEach(([M,Q])=>{let{mayaNode:W,unmountListener:X}=Q;fB(W,X)})}),fB=(B,M)=>{if(!s(B))return;let Q=B.children;for(let W=0;W<Q.length;W++){let X=Q[W];fB(X,X.unmountListener)}if(M&&M(B),N[B.nodeID])delete N[B.nodeID]},KM=()=>{if(!_B&&!S.currentIs("build"))LM.observe(document.body,{childList:!0,subtree:!0}),_B=!0},$B=(B,M,Q)=>{let W=x(M),X=x(B),Z=Q($(()=>X.value),$(()=>W.value)),_,H,F=!1;if(Z?.isMayaNodeGetter)_=()=>{if(F&&H)return H;return H=Z(),F=!0,H},_.isMayaNodeGetter=!0;else if(!Z||typeof Z==="string")_=Z||"";else throw`One of the child, ${Z} passed in ForElement is invalid.`;return{indexSignal:W,itemSignal:X,mappedChild:_}},wB=(B,M,Q)=>{if(M!==void 0&&M>=0&&Q){let W=M>B.length?B.length:M;B.splice(W,0,Q)}return B},RM=({subject:B,itemKey:M,map:Q,n:W,nthChild:X})=>{if(X&&W===void 0||W!==void 0&&W>-1&&!X)throw Error("Either both 'n' and 'nthChild' be passed or none of them.");let Z=X;if(X&&typeof X!=="string"){let U=X(),v=()=>U;v.isMayaNodeGetter=!0,Z=v}if(!M){let U=()=>wB(w(B).map(Q),W,Z);return V(B)?$(U):U()}let _=w(B);if(_.length&&typeof _[0]!=="object")throw Error("for mutable map, item in the list must be an object");let H=$(()=>{let U=w(B);if(!Array.isArray(U))throw`subject must be an array or signalled array, found ${JSON.stringify(B)}`;return U}),F=$((U)=>{if(!(U||[]).length||!(H.prevValue||[]).length)return H.value.map((z,R)=>$B(z,R,Q));return iB(H.prevValue||[],H.value,M).map((q,z)=>{let R=(U||[])[q.oldIndex];if(console.assert(q.type==="add"&&q.oldIndex===-1&&!R||q.oldIndex>-1&&!!R,"In case of mutation type 'add' oldIndex should be '-1', or else oldIndex should always be a non-negative integer."),R){if(q.type==="shuffle")R.indexSignal.value=z;if(q.type==="update")R.indexSignal.value=z,R.itemSignal.value={...q.value};return R}return $B(q.value,z,Q)})});return $(()=>wB(F.value.map((U)=>U.mappedChild),W,Z))};function zM({subject:B,isTruthy:M,isFalsy:Q}){let W=Y.Span({style:"display: none;"}),X=(Z)=>{if(w(B)){if(!M)return W;let F=M(B);return Z?w(F):F}if(!Q)return W;let H=Q(B);return Z?w(H):H};return V(B)?$(()=>X(!0)):X(!1)}var AM=({subject:B,caseMatcher:M,defaultCase:Q,cases:W})=>{let X=Y.Span({style:"display: none;"}),Z=Q&&Q(),_=(H)=>{let F=w(B),K=w(W),U=void 0;for(let[v,u]of Object.entries(K||{})){let q=M&&M(F,v),z=`${F}`===v;if(q||z){U=H?w(u()):u();break}}return U||Z||X};return V(B)?$(()=>_(!0)):_(!1)},d="http://www.w3.org/2000/svg",vM="http://www.w3.org/1998/Math/MathML",EM=(B,M)=>{if(M===d)return document.createElementNS(d,B);if(FB.includes(B))return document.createElementNS(vM,B);if(JB.includes(B))return document.createElementNS(d,B);return document.createElement(B)},bB=(B)=>B.startsWith("on"),OB=(B)=>B==="onmount"||B==="onunmount",SB=(B,M)=>bB(B)&&M===void 0,uB=(B,M)=>bB(B)&&!OB(B)&&typeof M==="function",yB=(B,M)=>OB(B)&&typeof M==="function",PM=(B,M)=>SB(B,M)||uB(B,M)||yB(B,M),VM=(B,M)=>{Object.entries(M).forEach(([Q,W])=>{if(SB(Q,W));else if(uB(Q,W)){let X=Q.slice(2);B.addEventListener(X,(Z)=>{if(X==="keypress")Z.preventDefault();W(Z)})}else if(yB(Q,W)){if(Q==="onmount"&&!S.currentIs("build")){let X=W;setTimeout(()=>X(B),0)}if(Q==="onunmount"){KM();let X=B.unmountListener;B.unmountListener=(Z)=>{if(W(Z),typeof X==="function")X(Z)}}}else console.error(`Invalid event key: ${Q} for element with tagName: ${B.tagName}`)})},GB=(B,M,Q)=>{let W=V(Q)?Q.value:Q,X=sB(M,W);if(typeof X==="boolean")if(X)B.setAttribute(M,"");else B.removeAttribute(M);else if(M==="value")B.value=X||"";else B.setAttribute(M,X||"")},fM=(B,M)=>{let Q={};Object.entries(M).forEach((X)=>{let[Z,_]=X;if(V(_))Q[Z]=_;GB(B,Z,_)});let W=C(()=>{Object.entries(Q).forEach((X)=>{let[Z,_]=X,H=_.value;if(!S.currentIs("run"))return;GB(B,Z,H)})});B.effects.push(W)},bM=(B)=>{if(!B||typeof B==="string")return document.createTextNode(UB(B||""));if(g(B)){let M=B();if(!s(M))throw Error(`Invalid maya-node-getter child. Type: ${typeof B}`);return M}throw Error(`Invalid child. Type of child: ${typeof B}`)},n=(B,M,Q)=>{let W=B.childNodes[Q],X=bM(M);if(W&&X)B.replaceChild(X,W);else if(X)B.appendChild(X);else console.error(`No child found for node with tagName: ${B.tagName}`)},OM=(B,M)=>{if(!M)return;if(PB(M)){let Z=C(()=>{let _=M.value,H=QB(_)?_:[_];H.forEach((K,U)=>n(B,K,U));let F=H.length;while(F<B.childNodes.length){let K=B.childNodes[F];if(K)B.removeChild(K)}});B.effects.push(Z)}let Q=M,W=g(Q)?[Q]:EB(Q)?Q.map((Z)=>BB(Z)?Z:Z):[],X=[];if(W.forEach((Z,_)=>{if(BB(Z))X.push({index:_,childSignal:Z});let H=w(Z);n(B,H,_)}),X.length)X.forEach(({index:Z,childSignal:_})=>{let H=C(()=>{let F=_.value;if(!S.currentIs("run"))return;n(B,F,Z)});B.effects.push(H)})},SM=(B,M)=>{let Q=void 0,W={},X={};return Object.entries(B).forEach(([Z,_])=>{if(Z==="children")if(VB(_))Q=_;else throw Error(`Invalid children prop for node with tagName: ${M}
 ...
 ${JSON.stringify(_)}`);else if(PM(Z,_))W[Z]=_;else X[Z]=_}),{children:Q,eventProps:W,attributeProps:X}},kB=(B,M,Q)=>{let W=()=>{let X=jB.getNewId(),Z=S.currentIs("mount")?document.querySelector(`[data-elem-id="${X}"]`):EM(B,Q);Z.nodeID=X,Z.effects=[],Z.unmountListener=()=>{Z.effects.forEach((F)=>F.dispose())};let _=VB(M)?{children:M}:M;if(!S.currentIs("run"))_["data-elem-id"]=Z.nodeID.toString();let H=SM(_,Z.tagName);if(VM(Z,H.eventProps),fM(Z,H.attributeProps),OM(Z,H.children),!S.currentIs("build"))Z.removeAttribute("data-elem-id");return Z};return W.isMayaNodeGetter=!0,W},uM=(B)=>(M)=>kB(B,M),yM=(B)=>(M)=>kB(B,M,d),kM=[...gB,...JB,...FB].reduce((B,M)=>{let Q=uM(M),W=M.split("-").map((X)=>X.charAt(0).toUpperCase()+X.slice(1)).join("");return B[W]=Q,B},{}),IM=Object.entries(NB).reduce((B,[M,Q])=>{return B[M]=yM(Q),B},{}),TM={For:RM,If:zM,Switch:AM},Y={...kM,...IM,...TM},xM=(B)=>{let M=w(B);if(!Array.isArray(M))return!1;return M.some((Q)=>V(Q))},IB=(B)=>{return(Q={})=>{let W={};for(let X of Object.keys(Q))if(Q[X]===void 0)delete Q[X];return Object.entries(Q).forEach((X)=>{let[Z,_]=X,H=typeof _==="function"?_:xM(_)?w(_):V(_)?_:$(()=>w(_));W[Z]=H}),B(W)}},f=(B)=>IB(B),kQ=f(({logoSrc:B,logoHref:M,logoSize:Q,labelComponent:W})=>{let X=A`${()=>Q?.value||32}`;return Y.A({class:"space-mono link black flex items-center justify-start",href:M,children:[Y.Img({src:B,height:X,width:X}),Y.If({subject:W,isTruthy:()=>W})]})}),IQ=f(({children:B})=>Y.Div({class:"flex items-center",children:B})),gM=f(({classNames:B,labelClassNames:M,href:Q,label:W,onClick:X})=>{let Z=A`flex justify-stretch pointer bg-white hover-bg-light-gray b--gray ba bw1 br-pill ${B}`,_=A`w-100 no-underline bg-transparent dark-gray pv2 ph3 ${M}`;return Y.Button({class:Z,onclick:X,children:Y.A({class:_,...Q?{href:Q}:{},children:W})})}),TQ=f(({className:B})=>Y.Div({class:A`bl b--moon-gray min-vh-20 ${B}`})),xQ=f(({classNames:B,url:M,size:Q})=>Y.A({class:B,target:"_blank",href:M||"https://github.com",children:[Y.Img({class:"ba b--none br-100",src:"https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",height:A`${()=>Q?.value||32}`,width:A`${()=>Q?.value||32}`})]})),NM=f(({classNames:B,hoverTitle:M,name:Q,size:W,onClick:X})=>{return Y.Span({class:A`material-symbols-rounded ${X?"pointer":""} ${B}`,style:A`font-size: ${W?.value??24}px; line-height: ${W?.value??24}px;`,title:M,onclick:X,children:Q})}),p=f(({classNames:B,colorCss:M,target:Q,isSelected:W,href:X,onClick:Z,label:_})=>{let H=r(M).then(`${M?.value}`,"dark-gray"),F=A`bg-${H}`,K=r(W).then("white",H),U=r(W).then(F,"transparent"),v=X||Z?"pointer":"";return Y.A({class:A`link underline ${v} ${K} ${U} ${B}`,target:Q,onclick:Z,children:_,...X?{href:X}:{}})}),CM=f(({classNames:B,children:M})=>{return Y.Div({class:A`dn db-ns fg3 pb3 pr2 max-h-80 overflow-y-scroll ${B}`,style:`
    ...
    `,children:M})}),HB=f(({classNames:B,linkColorCss:M,text:Q})=>{return Y.Span({class:B,children:Y.If({subject:Q.includes("##"),isFalsy:()=>Q,isTruthy:()=>Y.For({subject:Q.split("##"),map:(W,X)=>{let[Z="",_=""]=W.split("|");return Y.If({subject:X%2===0,isTruthy:()=>W,isFalsy:()=>p({colorCss:M||"theme-col",target:"_blank",label:Z,href:_})})}})})})}),mM=f(({classNames:B,titleClassNames:M,itemClassNames:Q,title:W,justifyRight:X,links:Z,onLinkClick:_,linkColorCss:H,bottomComponent:F})=>{return Y.Div({class:A`${()=>X?.value?"tr":""} ${B}`,children:[Y.P({class:A`space-mono mt0 f3 lh-solid ${M}`,children:W}),Y.Div(Y.For({subject:Z,itemKey:"title",map:(K,U)=>{let{title:v,href:u,isSelected:q}=K.props();return Y.Div({class:Q,children:[p({classNames:"ph2",colorCss:H,label:v,onClick:()=>_&&_(U.value),href:u,isSelected:q})]})}})),Y.If({subject:F,isTruthy:()=>F})]})}),WB=f(({classNames:B,contentClassNames:M,children:Q})=>{return Y.Div({class:A`w-100 bg-pale ${B}`,children:[Y.Div({class:A`mw8 center ${M}`,children:Q})]})}),hM=IB(({headerClassNames:B,headerTitle:M,headerComponent:Q,navbarClassNames:W,navbarComponent:X,contentClassNames:Z,contentTitle:_,contentComponent:H,scrollToTopCounterSignal:F})=>{let K;return C(()=>{if(F?.value)K.scrollTo({top:0})}),[Y.Div({class:A`mt3 mb5 flex-ns flex-wrap items-end ${B}`,children:[Y.H1({class:"mr3 mv2 mv1-ns",children:M}),Y.If({subject:Q,isTruthy:()=>Q})]}),Y.Div({class:"flex w-100 w-auto-ns",children:[CM({classNames:W,children:X}),Y.Div({onmount:(U)=>K=U,class:A`fg7 pb5 w-70-ns mw-100 w-auto-ns max-h-80 overflow-y-scroll
      ...
      they get to know yet another approach to solving current problems/limitations in the web development field.`],map:(B)=>Y.P(B)})}),iM=`
...
`,rM=["Good knowledge of HTML, CSS and JavaScript.","Basic knowledge of TypeScript.","Basic knowledge of using a Terminal in your machine and executing shell commands using it.",`MacOS or Linux (as preferred) operating system on your machine. However if you are working with
  Windows, you can use WSL on Windows for current purpose.`,"Terminal emulator of your choice on your machine."],nM=Y.Div([Y.P({class:"mt0",children:iM}),Y.Ul({children:Y.For({subject:rM,map:(B)=>Y.Li({class:"mb2",children:Y.If({subject:B.includes("##"),isFalsy:()=>B,isTruthy:()=>Y.Span(Y.For({subject:B.split("##"),map:(M,Q)=>{let[W="",X=""]=M.split("|");return Y.If({subject:Q%2===0,isTruthy:()=>M,isFalsy:()=>p({colorCss:"theme-col",target:"_blank",label:W,href:X})})}}))})})})})]),xB=()=>{let M=document.getElementById("hightlight-js-script");if(M)M.parentElement?.removeChild(M);document.body.appendChild(Y.Script({id:"hightlight-js-script",defer:!0,children:"hljs.highlightAll();"})())},l={FIRST_THINGS:"First things first",GETTING_FAMILIAR:"Getting familar with Maya's templating syntax",SYNTAX_RULES:"Basic syntax rules"},tM=[`Before jumping straight into complete tutorial, let's just ponder upon a few things first.
  ...
  would intuitively find a mapping pattern (from HTML to Maya).`],lM={HTML:{title:"HTML",code:`<div
  id="container"
  class="center pa3"
>
  <h3>My Blog Title</h3>
...
`}},aM=[`A Maya element is either a string or a (HTML equivalent template) function, for example,
 ...
 m.Button({ onclick: function someFn(){}, children: "click me" }).`],eM=[`You might still be confused about how to use this syntax. How to create components using this?
...
});`),E("The builder calls the default export during build. A page must be safe to evaluate in JSDOM.")),WQ=L(Y.H3({class:"black",children:"Elements are getters"}),j("Calling m.Div(...) returns an element getter. Calling that getter creates the element during build, or finds its matching static element during mount.","This is why a getter can be passed around as a child and reused by a component without needing a special template compiler."),J(`const title = m.H1("Welcome");
const content = m.Main([
  title,
  m.P({ id: "intro", children: "Read this first." }),
]);

const node = content();`),O("Element props",P("id, href, value, and data-* become attributes; pass application class values through the typed NoCSS css helper.","onclick, oninput, and other supported lower-case event keys register listeners.","onmount and onunmount are lifecycle callbacks for browser-only work and cleanup."))),NQ=L(Y.H3({class:"black",children:"Fragments group children"}),j("A fragment is a function that returns children rather than one wrapper element. Use it when a reusable piece of UI should contribute several siblings to its parent.","Use fragment() for a reusable sibling group and component() for one reusable Maya child. Keep plain functions for work that does not return Maya UI."),J(`import { component, fragment, m } from "@cyftec/maya/core";
...
);`)),XQ=L(Y.H3({class:"black",children:"Components add a typed boundary"}),j("Use component() when reusable Maya UI has named props. Maya preserves compatible reactive values and callback functions so the component can forward data into attributes or children without a rerender loop."),J(`import { component, m } from "@cyftec/maya/core";
import { css } from "./assets/styles.js";
...
Badge({ label: "New", tone: "success" });`),E("A component is your TypeScript function; m.Div and m.Span are the element factories it composes.")),YQ=L(Y.H3({class:"black",children:"Props describe data and behavior"}),j("Pass attributes directly to an element and named values to a component. Keep callback props as functions so the child can connect them to a DOM event.","For reactive values, pass a signal. Maya's element and attribute logic will track it and update the exact target when the signal changes."),J(`type CounterProps = {
...
});`)),CQ=L(Y.H3({class:"black",children:"Folders become routes"}),j("Brahma uses the view folder structure as the URL structure. Put page.ts inside a folder for an index page, and use a dotted page filename for a single named HTML file."),J(`dev/view/pages/page.ts             -> /index.html
dev/view/pages/docs/page.ts        -> /docs/index.html
dev/view/pages/docs/signals/page.ts -> /docs/signals/index.html
dev/view/pages/contact.page.ts     -> /contact.html`),P("Relative imports follow the source folder structure.","Relative links should point to the built route.","The generated script name follows the page route.")),_Q={TITLE:"Setup environment",PARA:"Firstly, make sure that below environments are ready.",STEPS:[{DESCRIPTION:"Install VS Code from ##here|https://code.visualstudio.com/## if not already done. You can also use any other editor of your choice. But you'll miss some good-to-have benefits.",ALERT:void 0,CODE:void 0},{DESCRIPTION:"Open your favourite terminal.",ALERT:void 0,CODE:void 0}]},$Q={TITLE:"Install packages",PARA:`After environment is set up, install below packages which is required
  ...
  'npm install -g bun' doesn't work well with Maya and its CLI currently.`,CODE:void 0},{DESCRIPTION:"Check if bun is installed properly using below command in your terminal.",ALERT:void 0,CODE:"bun"},{DESCRIPTION:"Install 'brahma' Cli for developing Maya apps using below command.",ALERT:void 0,CODE:"bun add -g @cyftec/brahma"},{DESCRIPTION:"Check if 'brahma' CLI is installed properly using below command in your terminal.",ALERT:void 0,CODE:"brahma"}]},wQ=[_Q,$Q],GQ=Y.Div({children:Y.For({subject:wQ,map:(B)=>Y.Div({class:"mb5",children:[Y.H3({class:"black",children:B.TITLE}),Y.P({class:"mt0",children:B.PARA}),Y.Ol({children:Y.For({subject:B.STEPS,map:({DESCRIPTION:M,ALERT:Q,CODE:W})=>Y.Li({class:"mb2",children:[HB({text:M}),Y.If({subject:Q,isTruthy:()=>E(HB({text:Q||""}))}),Y.If({subject:W,isTruthy:()=>J(W||"")})]})})})]})})}),mQ=L(Y.H3({class:"black",children:"Create a Maya app"}),j("Brahma creates a ready-to-run Maya project from one command. The app name becomes the new folder, and the optional mode chooses the kind of project scaffold you want to start with.","Start with a web app while learning. TypeScript pages become static HTML and page JavaScript, while the typed NoCSS source becomes the generated application stylesheet."),J("brahma create hello-maya"),Y.H3({class:"black",children:"Install and run"}),j("Run the below commands in terminal. It installs dependencies and then stages your app in watch mode for seeing the generated app UI and continously develop the app."),J(`cd hello-maya
brahma install
brahma stage`),O("Project modes",P("web — a normal web app with multiple page routes.","pwa — a progressive-web-app (PWA) scaffold with a typed manifest, icons, and service-worker entry point.","ext — a Chrome extension scaffold with a typed manifest, popup, content script, and service worker.")),E("The 'brahma create' command creates the selected scaffold and installs its NoCSS probe. 'brahma install' creates local package/config files from _karma/karma.ts and installs dependencies. Run it before the first 'brahma stage' and whenever generated files or dependencies need synchronization."),J(`brahma create hello-pwa --pwa
brahma create hello-extension --ext`)),HQ=L(Y.H3({class:"black",children:"Find your way around the app"}),j("A Maya project keeps application source under dev. The configured appViewDir contains pages and public assets. Directories beginning with the configured @ delimiter are source-only and are not copied as routes.","The standard web scaffold uses dev/view/pages/page.ts for the home page, an about/page.ts file for a folder route, and contacts.page.ts for a dotted page route."),J(`hello-maya/
├── _karma/karma.ts
└── dev/
    ├── controllers/
    ├── models/
...
]);`),O("Three phases",Y.Ol({children:[Y.Li({class:"mb2",children:"Build: Brahma runs you app view (page.ts) in JSDOM and builds static HTML multi-page-app (MPA)."}),Y.Li({class:"mb2",children:"Mount: the page script finds those nodes using data-elem-id markers."}),Y.Li({class:"mb2",children:"Run: events and signal effects are now allowed to change the DOM."})]})),J("TypeScript page -> static HTML with markers -> mount existing DOM -> run reactive effects")),JQ=L(Y.H3({class:"black",children:"Configure the project in karma.ts"}),j("Karma is a TypeScript object exported from _karma/karma.ts. It is the source Brahma reads for source and output paths, page and stylesheet names, package metadata, editor settings, Git ignores, and the generated TypeScript configuration.","Generated package.json and tsconfig.json are disposable projections of Karma. Change the typed Karma source, then run brahma install to synchronize them."),J(`import type { Karma } from "./types.js";
...
};`),O("The important distinction",E("_karma/karma.ts is the source configuration. package.json, tsconfig.json, .gitignore, VS Code settings, and Zed settings are generated or synchronized from it. TypeScript is pinned exactly to 7.0.2; do not replace it with a range or downgrade it to hide an error."))),dQ=L(Y.H3({children:"Style the app with NoCSS"}),j("NoCSS is Maya's application styling system. You author a typed styles.ts module, use its css helper in Maya elements, and let Brahma generate styles.css from the class names collected while it builds the pages.","The stylesheet filename and assets directory come from _karma/karma.ts. In the standard web scaffold, the source is dev/view/pages/assets/styles.ts and the generated asset is stage/assets/styles.css."),J(`import {
  getCss,
  type AppClassNames,
...
export const css = getCss<ClassName>();`),O("Use the typed helper",J(`import { css } from "./assets/styles.js";
...
});`),P("Use css for every class, including one static class.","Use css.when for boolean style states.","Use css.cases when one subject selects among several style states.","Use css.ifNullable only when null or undefined needs a static fallback.","Add missing declarations and reusable groups to the same styles.ts.")),E("Coding agents may author application styles only through NoCSS. They must not write a CSS file, inline style, style element, injected CSS, raw class string, or another styling dependency. This restriction does not apply to humans.")),cQ=L(Y.H3({class:"black",children:"Let Brahma build and serve"}),j("Brahma is Maya's Bun-first CLI. It creates scaffolds, installs generated project files, builds pages, compiles NoCSS, watches source changes, and produces deployable output.","During stage, Brahma clears the NoCSS registry, statically builds the pages, generates the used stylesheet, watches the configured source directory, and serves the staging folder."),J(`brahma help
brahma stage
brahma publish
brahma reset
brahma reset --stylesheet
brahma uninstall`),O("A useful development loop",P("Edit a page or shared component inside dev.","Express every application class through the typed NoCSS css helper.","Run brahma stage to build and serve the current app.","Open the printed localhost address and test the browser behavior.","Run brahma publish when you want minified production output.")),J(`brahma stage  # build, watch, and serve
brahma publish # build production files`)),sQ=L(Y.H3({class:"black",children:"Static and reactive pages"}),j("A static Maya page uses ordinary strings and elements. Brahma can build it completely, and the browser only needs the generated page script if it has interactions.","A reactive page adds signals. The initial value still becomes static HTML, but the mounted page also registers effects that update individual nodes or attributes as state changes."),J(`const message = "Always the same";
...
]);`),E("Both kinds of page are built ahead of time. Reactive does not mean the whole page is re-rendered. It only mean that when prebuilt reactive page is loaded and mounted in the browser, the DOM gets mutated when the signalled attributes or nodes changes")),FQ=L(Y.H3({class:"black",children:"A signal is changing state"}),j("A signal is a small state container with a .value. Read its value to use the current state, and assign a new value to notify the computations that read it.","Maya re-exports signal helpers from @cyftec/maya/signals, including signal, derive, effect, and tmpl."),J('const name = signal("Ada");\nconst greeting = m.P({ children: tmpl`Hello, ${name}!` });\n\nname.value = "Grace";'),P("signal(value) creates mutable source state.","Reading .value inside a reactive computation records a dependency.","Assigning .value causes dependent effects and derived signals to run.")),UQ=L(Y.H3({class:"black",children:"Effects run when dependencies change"}),j("effect() runs its callback immediately and again whenever a signal read by that callback changes. Maya uses this mechanism internally for reactive children and attributes.","Usually, let an element consume the signal directly. Use an explicit effect when you need an imperative side effect such as logging or synchronizing a browser API."),J(`const count = signal(0);

effect(() => {
  console.log("The count is", count.value);
});

count.value = 1; // the effect runs again`),E("Keep signal reads inside the effect callback so the dependency is tracked.")),jQ=L(Y.H3({class:"black",children:"Derived signals calculate values"}),j("derive() creates read-only state from other signals. It recalculates when the signals read in its callback change, which keeps display logic out of event handlers."),J(`const first = signal("Ada");
const last = signal("Lovelace");
const fullName = derive(() =>
  \`\${first.value} \${last.value}\`,
);

m.P({ children: fullName });`),O("Choose the right primitive",P("Use signal for state an event handler changes.","Use derive for a value calculated from state.","Use effect for an imperative action after a dependency changes.","Use tmpl for a convenient derived string in text or attributes."))),qQ=L(Y.H3({class:"black",children:"Signals connect UI to state"}),j("Pass a signal as a child or attribute when the target should update. Use m.If, m.Switch, and m.For when the shape of the child list itself depends on state.","For a mutable list of objects, give m.For an itemKey. Maya can preserve each mapped DOM node while updating the derived item and index signals."),J(`const tasks = signal([
...
./node_modules/.bin/tsc --version`)),PQ=L(Y.H3({children:"NoCSS is the application styling system"}),j("The configured styles.ts is the single authored stylesheet source. It exports application overrides, responsive constraints, compound classes, a complete ClassName type, and the app's typed css helper.","Brahma clears the class registry, statically renders every page, collects names passed through css, and emits only the matching rules as a minified styles.css in the output assets directory."),J(`import {
  getCss,
  type AppClassNames,
  type AtomicClassOverrides,
  type BaseClassName,
...
export const css = getCss<ClassName>();`),O("Apply and collect classes",J(`import { css } from "./assets/styles.js";
...
m.Button({
  ...
  children: "Save",
});`),P("css combines static, optional, validated, and signalled class phrases.","css.when registers both boolean outcomes during the static build.","css.cases registers every declared subject outcome and an optional default.","css.ifNullable uses a static fallback only for null or undefined.","Use responsive -ns, -m, and -l classes or the matching override groups.")),O("Mandatory rule for coding agents",...j("Coding agents must author CSS-based application styles only through NoCSS. They must pass every class through the app's typed css helper and must not create or edit a stylesheet, inline style, style element, injected CSS, or an additional styling dependency.","This restriction is for coding agents, not human contributors. Existing human-authored CSS may remain when unrelated, but an agent changing a style must move that behavior into NoCSS. Canvas drawing commands remain valid for canvas pixels; the canvas element and surrounding DOM use NoCSS."),E("If the public NoCSS API cannot represent a requirement, treat that as a framework capability gap. Do not silently bypass the framework with another styling approach."))),VQ=L(Y.H3({class:"black",children:"Maya's signal integration"}),j("Maya does not implement a second signal system. It re-exports @cyftec/signals through @cyftec/maya/signals and uses the same primitives internally for reactive attributes, children, derived values, and effects.","The important Maya behavior is where those signals connect: each element stores the effects associated with its attributes and children, and those effects are disposed when the element unmounts."),J(`import { signal, derive, effect } from "@cyftec/maya/signals";

const count = signal(0);
const doubled = derive(() => count.value * 2);

effect(() => console.log(doubled.value));`),E("Use the public Maya signal import in app code. The dependency package supplies the implementation; Maya supplies the DOM integration.")),fQ=L(Y.H3({class:"black",children:"The default HTML page"}),j("A page is a Maya HTML getter with head metadata, a body, and the page-local script emitted by Brahma. The script must be deferred so the static DOM exists before mount begins."),J(`import { m } from "@cyftec/maya/core";
import { css } from "./assets/styles.js";
...
};`),E("Read document and window only in browser-safe code. A page is evaluated in JSDOM during build.")),OQ=L(Y.H3({class:"black",children:"The UI toolkit"}),j("The current Maya toolkit exports query(). It wraps a GET fetch request in signal state so a component can react to loading, data, and error changes."),J(`import { query } from "@cyftec/maya/toolkit";
...
users.error.value;
users.abortQuery();
users.clearCache();`),O("What it is not",P("It is not a full data cache or retry framework.","It does not define a component-level loading UI for you.","It does not replace fetch or native browser error handling."))),SQ=[{title:"Overview",topics:[{title:"Getting familiar",article:BQ},{title:"Prerequisites",article:nM},{title:"Installation",article:GQ},{title:"App structure",article:HQ},{title:"Brahma, Karma & Maya",article:DQ},{title:"Karma config",article:JQ},{title:"TypeScript 7.0.2",article:EQ}]},{title:"Brahma (CLI)",topics:[{title:"Why the CLI?",article:LQ},{title:"brahma create",article:KQ},{title:"brahma install",article:RQ},{title:"brahma uninstall",article:zQ},{title:"brahma reset",article:vQ},{title:"brahma publish",article:AQ}]},{title:"Maya",topics:[{title:"Syntax",article:MQ},{title:"Overview",article:QQ},{title:"Element",article:WQ},{title:"component",article:XQ},{title:"Props",article:YQ},{title:"Page",article:ZQ}]},{title:"NoCSS",topics:[{title:"Styling applications",article:PQ}]},{title:"Signal",topics:[{title:"What is signal?",article:FQ},{title:"Custom implementation",article:VQ},{title:"Effect",article:UQ},{title:"Derived signals",article:jQ},{title:"Signal for mutating list",article:qQ}]},{title:"Toolbox",topics:[{title:"Default HTML page",article:fQ},{title:"Router",article:bQ},{title:"UI toolkit",article:OQ}]}],uQ=pM({htmlTitle:"Docs - Maya",pageTitle:"Docs",headElements:[Y.Link({rel:"stylesheet",href:"https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.11.1/styles/agate.min.css"}),Y.Script({src:"https://cdn.jsdelivr.net/gh/highlightjs/cdn-release@11.9.0/build/highlight.min.js"})],chapters:SQ}),yQ=()=>{S.start("mount"),jB.resetIdCounter(),uQ(),S.start("run")};yQ();
