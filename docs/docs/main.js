...
const node = content();`),V("Element props",P("id, href, value, and data-* become attributes; pass application class values through the typed NoCSS css helper.","onclick, oninput, and other supported lower-case event keys register listeners.","onmount and onunmount are lifecycle callbacks for browser-only work and cleanup."))),NQ=L(Y.H3({class:"black",children:"Fragments group children"}),U("A fragment is a function that returns children rather than one wrapper element. Use it when a reusable piece of UI should contribute several siblings to its parent.","Use fragment() for a reusable sibling group and component() for one reusable Maya child. Keep plain functions for work that does not return Maya UI."),J(`import { component, fragment, m } from "@cyftec/maya/core";
...
);`)),XQ=L(Y.H3({class:"black",children:"Components add a typed boundary"}),U("Use component() when reusable Maya UI has named props. Maya preserves compatible reactive values and callback functions so the component can forward data into attributes or children without a rerender loop."),J(`import { component, m } from "@cyftec/maya/core";
import { css } from "./assets/styles.js";
...
Badge({ label: "New", tone: "success" });`),E("A component is your TypeScript function; m.Div and m.Span are the element factories it composes.")),YQ=L(Y.H3({class:"black",children:"Props describe data and behavior"}),U("Pass attributes directly to an element and named values to a component. Keep callback props as functions so the child can connect them to a DOM event.","For reactive values, pass a signal. Maya's element and attribute logic will track it and update the exact target when the signal changes."),J(`type CounterProps = {
...
});`)),CQ=L(Y.H3({class:"black",children:"Folders become routes"}),U("Brahma uses the view folder structure as the URL structure. Put page.ts inside a folder for an index page, and use a dotted page filename for a single named HTML file."),J(`dev/view/pages/page.ts             -> /index.html
...
brahma create hello-extension --ext`)),HQ=L(Y.H3({class:"black",children:"Find your way around the app"}),U("A Maya project keeps application source under dev. The configured appViewDir contains pages and public assets. Directories beginning with the configured @ delimiter are source-only and are not copied as routes.","The standard web scaffold uses dev/view/pages/page.ts for the home page, an about/page.ts file for a folder route, and contacts.page.ts for a dotted page route."),J(`hello-maya/
├── _karma/karma.ts
└── dev/
    ├── controllers/
    ├── models/
...
};`),E("Read document and window only in browser-safe code. A page is evaluated in JSDOM during build.")),OQ=L(Y.H3({class:"black",children:"The UI toolkit"}),U("The current Maya toolkit exports query(). It wraps a GET fetch request in signal state so a component can react to loading, data, and error changes."),J(`import { query } from "@cyftec/maya/toolkit";
